...
Color *evaluateOnePixel(Image *image, int row, int col)
{
	...
	int blue=image->image[row][col].B,message=(blue&1)*255;
...
Image *steganography(Image *image)
{
	...
	decoded->image=(Color**)malloc(sizeof(Color*)*decoded->rows);
	for (int i=0;i<image->rows;++i)
	{
		decoded->image[i]=(Color*)malloc(sizeof(Color)*decoded->cols);
		for (int j=0;j<image->cols;++j)
		{
			Color *color=evaluateOnePixel(image,i,j);
			decoded->image[i][j]=*color;
			free(color);
		}
	}
	return decoded;
